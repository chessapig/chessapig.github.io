...
## Quiver varieties
These polygon spaces serve as the concrete realizations of even fancier ideas. For example, you can represent the direction of each stick as a point in $S^2 \cong \mathbb{CP}^1$, or as a 1-dimensional complex subspace of $\mathbb{C}^2$. The polygon moduli space is a collection of $n$ 1-dimensional subspaces of $\mathbb{C}^2$, up to linear transforms $SL(2,\mathbb{C})$ of $\mathbb{C}^2$. (This is a manifestation of the Kempf-Ness theorem. The symplectic reduction by the real group $SU(2)$ is isomorphic to the GIT quotient by its complexification $SL(2,\mathbb{C})$). Let's draw the vector spaces in the above construction: Represent each vector space by a node, and connect them by an arrow whenever one maps to another. This is called a *quiver*.
...
## Hyperkahler analogue

Things get truly useful after complexifying the quiver. We double the dimension by adding *backwards* arrows, from the core 2-dimensional space to the satellite 1-dimensional spaces. The resulting quiver variety is called the *nakajima quiver variety*. Where the ordinary quiver variety is Kahler, its complexification is *Hyperkahler*. Locally, the new space is the cotangent bundle of a quiver variety. Globally, where a Kahler manifold is built on complex, a hyperkahler manifold is built on the complexifed complex numbers, the *quaterionions*. We call this hyperkahler analogue of the polygon space the creative name, a hyperpolygon space. 
...
Remember how polygon spaces were secretly moduli of bundles over a punctured sphere? If god is nice to us, hyperpolygon spaces should be the complexification of the moduli of vector bundles, which is called the moduli space of Higgs bundles. It is a blessed day, because that's exactly what happens! We can generalize this to vector bundles and Higgs bundles over higher genus surfaces by adding loops to our quiver. Following our astrological names, we call this a  comet shaped. (See [Rayan and Schaposnik, Moduli space of generalized hyperpolygons](https://arxiv.org/pdf/2001.06911.pdf))

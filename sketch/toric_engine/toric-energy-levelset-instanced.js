...
p.setup=function(){
	...
	const test = p.createCanvas(1000,1000);
	test.parent("toricSphere");

	//bkg = p.color(11, 13, 31);
	bkg=p.color(BKG);
	//color1=color(188, 182, 207);
	color1=p.color(WHITE);
	//color2=p.color(87, 4, 99);
	color2=p.color(GREEN);
	//color3=p.color(230, 247, 181);
	color3=p.color(YELLOW);

	p.background(bkg);

	p.stroke(255);
	p.strokeWeight(.01);
	//p.strokeWeight(1); //for SVG
	p.fill(255);
	t=0;
	x=0;
	y=0;
	...
	for(var i = 0; i <= maxSHOs; i++){
		t=i/maxSHOs;
		col=p.color(255*(1-t*.8));
		os[i]=new DoubleOsscilator(
			1, 	p.sqrt(t*0.8),		perminantOffset,
			1,	p.sqrt((1-t)*0.8),	perminantOffset,
			p.lerpColor(color1,color2,t));
	} 
	updatePerturb();
	...
	settings = QuickSettings.create(10,10, "click / double tab to collapse")
		...
		.addRange("set reletive phase offset",-3,0.001,-3,.01,function(value) {
			offset=p.pow(10,value)-p.pow(10,-3);
			for(var i = 0; i <= maxSHOs; i++){
				t=i/maxSHOs;
...
function updatePerturb(){
	let numP=p.pow(10,perturb)-p.pow(10,perturbMin);;
	for(var i = 0; i <= maxSHOs; i++){
		os[i].x.dM =  numP*p.cos(perturbAngle*p.PI/2);
		os[i].y.dM =  numP*p.sin(perturbAngle*p.PI/2);
	} 
}
...
p.draw = function(){
	...
	p.scale(p.height/2);
	let bkg_color = p.color(BKG);
	bkg_color.setAlpha(bkgOpactity);
	p.background(bkg_color);


	if(doGraph){
		updateGraph();
		p.image(graph,-1,-1,.75,.75);
	}
...
function doDraw(i){
	if(numSHOs==1){return i==50}
		...
		return i%p.floor(maxSHOs/numSHOs)==0 || i==maxSHOs;
	}
}
...
class DoubleOsscilator{
	...
	draw(){ //allow different hcoice of canvas
		...
		p.stroke(this.color);
		if(doLine){
			p.line(this.x.q,this.y.q, this.x.qLast, this.y.qLast);
		}else {
			p.circle(this.x.q,this.y.q,0,0);
		}
		col.setAlpha(255);
		p.pop();
	}
	...
	drawPoint(){
		...
		p.stroke(color3);
		p.fill(this.color);
		p.circle(this.x.q,this.y.q,.05,.05);
		p.pop();
	}
}
...
class Osscilator{
	...
	move(t){ //eolve by time t
		...
		this.q=this.r*p.sin((this.phase+this.phaseOffset)*2*p.PI);
	}

	getH(){
		return 4*p.pow(this.r,2)
	}
	getMass(){
...
function drawGrid(g,scale){
	g.background(p.color(DARK));
	g.strokeWeight(0.01);
	g.stroke(p.color(WHITE));
...
function setupGraph(){
	graph = p.createGraphics(200,200);
...
var toricSphere1 = new p5(toricSphere);
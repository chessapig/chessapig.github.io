...
//returns 3 vector containing the position on the sphere of the uv coordinates of the screen.
function worldToComplex(uv, viewVector){
...
class Zeros{
    ...
    //given set of world coordinates, sets the complex coordinates
    fromWorld(worlds,viewVector){
        let zs = [];
        const [xBasis, yBasis, view] = getFrame(viewVector);
    ...
    fromDraggables(wilson){
        ...
        for (let i = 0; i < this.degree; i++) {
            ...
            worlds.push(p.createVector(world[0],world[1]))
        }
        this.fromWorld(worlds,wilson.viewVector)
    }
